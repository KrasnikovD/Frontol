...
function SetOptPrice(){
    ...
    if (FD.cardValues != ''){
        frontol.actions.showMessage("Введена карта!!!" + "\n" + "Добовление/Изменение товара запрещена!!! " + summPotratsAdd);
    } else {   
        for (
            FD.position.index = 1;
...
GetCardByCode();
 ...
 //frontol.actions.wait("Получение данных...", SET_TIME_WAIT);
        if(CardDeleted.text === 1){
            frontol.actions.showMessage("Карта с номером: " + FD.cardValues + " заблокирована!!!"+"\n"+"За информацией обратитесь к Администратору");
            WShell = new ActiveXObject("WScript.Shell");
            WShell.SendKeys("^{F12}");
        } else {
    ...
    if (xhttp.readyState == 4 && xhttp.status == 200){
        ...
        xmlDoc.loadXML(xhttp.responseText);
         if (xmlDoc.parseError.errorCode != 0) {
            ...
            CardDeleted  = xmlDoc.selectSingleNode("//AG4Response//HolderInfo//Deleted");
